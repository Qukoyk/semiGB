...
'''
# レバー格納関数
def leverIn(leverSide):
    GPIO.output(leverSide, GPIO.HIGH)
    #GPIO.output(houseLight, GPIO.HIGH)
    pass

# レバー引き出し関数
def leverOut(leverSide):
    GPIO.output(leverSide, GPIO.LOW)
    ...
    while timeNow - timeStart < timeMax:
        ...
        while react != x: # 加速ループ → この部分に絞る
            if sideLeft >= sideThreshold:
                leverIn(leverLeftMove)
                leverOut(leverRightMove)
            elif sideRight >= sideThreshold:
                leverIn(leverRightMove)
                leverOut(leverLeftMove)
